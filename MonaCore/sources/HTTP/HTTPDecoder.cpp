...
UInt32 HTTPDecoder::onStreamData(Packet& buffer, Socket& socket) {
	...
	do {
		...
		// HEADER
		UInt32 toBufferize = parseHeader(buffer, socket);
		if (toBufferize)
			return toBufferize;
		...
		// CHUNK (https://tools.ietf.org/html/rfc7230#section-4.1)
		if (!_length && !_ex && _stage == CHUNKED) {
			for (;;) {
				...
				if(buffer.data()!=packet.data())
					break; // at least one char gotten (to accept double \r\n\r\n and \r\n after playload data!)
				packet += 2;
				buffer += 2;
			}
		...
		if (_pReader) { // POST media streaming (publish)
			...
			if ((!_length && _stage < CHUNKED)) {
				_pReader->flush(*this);
				delete _pReader;
				_pReader = NULL;
			}
		}
		if (!_pReader && (_pHeader || _stage != CMD)) // can be CMD on end of chunked transfer-encoding (https://tools.ietf.org/html/rfc7230#section-4.1.2)
...
UInt32 HTTPDecoder::parseHeader(Packet& buffer, Socket& socket) {
	...
	while (_stage<BODY) {
		...
		if (buffer.size() < 2) {
			// useless to parse if we have not at least one line which to carry \r\n\r\n!
