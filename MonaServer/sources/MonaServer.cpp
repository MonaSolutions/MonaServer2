...
MonaServer::MonaServer(const Parameters& configs, TerminateSignal& terminateSignal) : _pState(NULL),
	Server(configs.getNumber<UInt16>("cores")), servers(sockets), _data(this->poolBuffers),_terminateSignal(terminateSignal),
	...
	onServerConnection = [this](ServerConnection& server) {
		...
		LUABroadcaster::AddServer(_pState,servers, server.address);
		LUABroadcaster::AddServer(_pState, server.isTarget ? servers.targets : servers.initiators, server.address);
		...
		shared<ServerMessage> pMessage(new ServerMessage(".",poolBuffers));
		for(const Service* pService : _servicesRunning)
			pMessage->packet.writeString(pService->path);
		server.send(pMessage);
	};

	onServerMessage = [this](ServerConnection& server, const string& name, BinaryReader& reader) {
		if (handler == ".") {
	...
	onServerDisconnection = [this](const Exception& ex,const ServerConnection& server) {
		...
		LUABroadcaster::RemoveServer(_pState, servers, server.address);
		LUABroadcaster::RemoveServer(_pState, server.isTarget ? servers.targets : servers.initiators, server.address);
...
void MonaServer::onStart() {
	...
	PersistentData::ForEach forEach([this,&firstData](const string& path, const UInt8* value, UInt32 size) {
		...
		if (String::Split(newPath, "/", forEach, SPLIT_IGNORE_EMPTY | SPLIT_TRIM) == string::npos)
			return;
			...
			if (pBroadcaster) {
				for (ServerConnection* pServer : *pBroadcaster) {
					if (pServer->protocolAddress(ex, protocol, address)) {
						...
						if (ex)
							SCRIPT_WARN("Redirection address, ",ex)
					} else
						SCRIPT_ERROR("Redirection address, ",ex)
				}
				return;
			} 
...
void MonaServer::onRendezVousUnknown(const string& protocol,const UInt8* id,set<SocketAddress>& addresses) {
	...
	SCRIPT_BEGIN(_pState)
		SCRIPT_FUNCTION_BEGIN("onRendezVousUnknown",_pService->reference())
			...
			SCRIPT_WRITE_BINARY(id,Entity::SIZE)
			SCRIPT_FUNCTION_CALL
...
void MonaServer::onConnection(Exception& ex, Client& client,DataReader& parameters,DataWriter& response) {
	...
	SCRIPT_BEGIN(_pState)
		SCRIPT_FUNCTION_BEGIN("onConnection",pService->reference())
			...
			SCRIPT_FUNCTION_CALL
			if (SCRIPT_FUNCTION_ERROR)
				ex.set<Ex::Application::Error>(strlen(SCRIPT_FUNCTION_ERROR) > 0 ? SCRIPT_FUNCTION_ERROR : "client rejected");
...
void MonaServer::onAddressChanged(Client& client,const SocketAddress& oldAddress) {
	SCRIPT_BEGIN(loadService(client))
		SCRIPT_MEMBER_FUNCTION_BEGIN(Client, client, "onAddressChanged")
			SCRIPT_WRITE_STRING(oldAddress.c_str())
			SCRIPT_FUNCTION_CALL
		SCRIPT_FUNCTION_END
	SCRIPT_END
}
...
bool MonaServer::onInvocation(Exception& ex, Client& client,const string& name,DataReader& reader, UInt8 responseType) {
	bool found(false);
	SCRIPT_BEGIN(loadService(client))
		SCRIPT_MEMBER_FUNCTION_BEGIN(Client,client,name.empty() ? "onMessage" : name)
			ScriptWriter writer(_pState);
			reader.read(writer);
			found=true;
			SCRIPT_FUNCTION_CALL
...
bool MonaServer::onFileAccess(Exception& ex, Client& client, Client::FileAccessType type, DataReader& parameters, File& file, DataWriter& properties) { 
	...
	if (file.isFolder()) {
		...
		ex.set(Exception::PERMISSION, "Impossible to access to ",file," ressource through the ",client.path," application, this resource is managed by ",file.parent()," application");
		return false;
	}
		...
		SCRIPT_MEMBER_FUNCTION_BEGIN(Client,client,type==Client::FileAccessType::READ ? "onRead" : "onWrite")
			...
			if (SCRIPT_FUNCTION_ERROR) {
				...
				if (SCRIPT_NEXT_TYPE == LUA_TNIL) {
					...
					string name;
					if (!file.setName(SCRIPT_READ_STRING(file.name()))
