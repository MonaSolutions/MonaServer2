...
void LUAGroup::Init(lua_State* pState, Group& group) {
	...
	string hex;
	lua_pushstring(pState, Mona::Util::FormatHex(group.id, Entity::SIZE, hex).c_str());
...
int LUAGroup::IndexConst(lua_State *pState) {
	SCRIPT_CALLBACK(Group,group)
		...
		if (name) {
			if(strcmp(name,"id")==0) {
				...
				SCRIPT_WRITE_BINARY(group.id,Entity::SIZE);
