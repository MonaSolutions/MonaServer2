...
void LUAMember::Init(lua_State* pState, Peer& member) {
	...
	string hex;
	lua_pushstring(pState, Mona::Util::FormatHex(member.id, Entity::SIZE, hex).c_str());
...
int LUAMember::IndexConst(lua_State *pState) {
	SCRIPT_CALLBACK(Peer,member)
		...
		if(name) {
			if (strcmp(name,"id")==0) {
				...
				SCRIPT_WRITE_BINARY(member.id,Entity::SIZE)
