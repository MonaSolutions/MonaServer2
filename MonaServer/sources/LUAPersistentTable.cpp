...
void LUAPersistentTable::Init(lua_State* pState, LUAPersistentTable& table) {
	...
	lua_pushcfunction(pState,&LUAPersistentTable::NewIndex);
...
int LUAPersistentTable::Index(lua_State *pState) {
	SCRIPT_CALLBACK(LUAPersistentTable, table)
...
int LUAPersistentTable::NewIndex(lua_State *pState) {
	SCRIPT_CALLBACK(LUAPersistentTable, table);
		const char* name = SCRIPT_READ_STRING(NULL);
		if (name) {
			Int8 removing = SCRIPT_NEXT_TYPE==LUA_TNIL ? 1 : 0;
			string path;
			FileSystem::MakeFolder(String::Assign(path, table.path, '/', name));
			...
			if (removing>=0) {
				if (SCRIPT_NEXT_TYPE==LUA_TTABLE) {
					...
					if (success) {
						...
						lua_rawset(pState, -4);
						if (ex)
							SCRIPT_WARN("Database entry ", path, " writing, ", ex)
					} else
						SCRIPT_ERROR("Database entry ", path, " writing, ", ex)
				}
			}
...
const UInt8* LUAPersistentTable::Serialize(lua_State* pState, int index, UInt32& size) {
	...
	switch (lua_type(pState, index)) {
		...
		case LUA_TTABLE: {
			string pointer;
			String::Assign(pointer, lua_topointer(pState, index));
			size = pointer.size();
